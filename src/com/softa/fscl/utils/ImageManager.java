...
public class ImageManager {
	...
	/** 图片加载队列，后进先出 */
	private Queue<ImageRef> mImageQueue = new LinkedList<ImageRef>();
	...
	private void sendRequest() {
		...
		if (mImageLoaderIdle && mImageQueue.size() > 0) {
			ImageRef imageRef = mImageQueue.poll();
				...
				if (msg.obj != null && msg.obj instanceof ImageRef) {
					...
					if (bitmap != null) {
						...
						try {
							...
							if (data != null) {
								if(imageRef.width==0){
									Log.e("iszero", imageRef.filePath);
								}
								tBitmap=PictureUtils.getImageThumbnail(data, imageRef.width);
//								BitmapFactory.Options opt = new BitmapFactory.Options();
//								opt.inSampleSize = 1;
//								opt.inJustDecodeBounds = true;
...
//										data.length, opt);
//								 
//									opt.inSampleSize = PictureUtils.computeSampleSize(opt, -1,(int)(1.5*imageRef.width*imageRef.width));
//								opt.inJustDecodeBounds = false;
...
//													ThumbnailUtils.OPTIONS_RECYCLE_INPUT);
//								} else {
							//	Log.d("url", imageRef.url);
									bitmap = tBitmap;
									Log.d("图片宽度",""+bitmap.getWidth());
									tBitmap = null;
	...
	private void setImageBitmap(ImageView imageView, Bitmap bitmap,
		...
		//	Log.d(getClass().toString(), "从网络下载bitmap赋值 width="+bitmap.getWidth());
