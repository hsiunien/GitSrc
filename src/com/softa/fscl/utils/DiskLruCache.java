...
public class DiskLruCache {
	...
	public static DiskLruCache openCache(Context context, File cacheDir, long maxByteSize) {
		...
		if (cacheDir.isDirectory() && cacheDir.canWrite()) {
			return new DiskLruCache(cacheDir, maxByteSize,context);
		}
	 ...
	 * @param maxByteSize
	 */
	private DiskLruCache(File cacheDir, long maxByteSize,Context context) {
		mCacheDir = cacheDir;
	...
	public Bitmap get(String key) {
		synchronized (mLinkedHashMap) {
			try {
				...
				Log.e(getClass().toString(), "内存溢出");
				return BitmapFactory.decodeResource(mContext.getResources(), R.drawable.loading);
