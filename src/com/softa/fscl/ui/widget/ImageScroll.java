...
public class ImageScroll extends ViewPager {
	...
	int oldIndex = 0;
	int curIndex = 0;
	private int state = 0; // 0 已停止滚动 1 开启滚动
	...
	public void start(Activity mainActivity, List<View> imgList,
		...
		if (scrollTime != 0 && imgList.size() > 1) {
			...
			// new FixSpeed(mActivity).setDuration(this, 700);
	...
	private class MyPagerAdapter extends PagerAdapter {
		...
		public Object instantiateItem(View v, int i) {
			...
			isInViewPager((ViewPager) v, mListViews.get(i % mListViews.size()));
			((ViewPager) v).addView(mListViews.get(i % mListViews.size()), 0);
			return mListViews.get(i % mListViews.size());
		}
		//防止进去然后向回滚动报错
		private  boolean isInViewPager(ViewPager viewPager,View v){
			for (int i = 0; i < viewPager.getChildCount(); i++) {
				if(viewPager.getChildAt(i).equals(v)){
					//Log.d(getClass().toString(),"有"+i);
